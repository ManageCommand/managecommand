...
class RunnerConfig:
    ...
    max_retries: int = 3
    allow_http_hosts: list[str] = field(
        default_factory=lambda: DEFAULT_ALLOW_HTTP_HOSTS.copy()
    )
...
def load_config() -> RunnerConfig:
    ...
    config = RunnerConfig(
        ...
        allow_http_hosts=list(
            getattr(
                settings, "MANAGECOMMAND_ALLOW_HTTP_HOSTS", DEFAULT_ALLOW_HTTP_HOSTS
            )
        ),
